...
def edgeDectector(pixels_magnitude : np.ndarray, pixels_direction : np.ndarray, weight : np.float64 = 1) -> np.ndarray:
    ...
    for y in range(0, len(pixels_borders)):                                         
        for x in range(0, len(pixels_borders[0])):          
            ...
            if(convertToRad(-22.5) <= pixels_direction[y][x] <= convertToRad(22.5)):
                if(getColorAt(pixels_magnitude, x-1, y) * weight < getColorAt(pixels_magnitude, x, y)
                   and getColorAt(pixels_magnitude, x+1, y) * weight < getColorAt(pixels_magnitude, x, y)):
                    pixels_borders[y][x] = 255
            elif(convertToRad(22.5) < pixels_direction[y][x] <= convertToRad(67.5)):
                if(getColorAt(pixels_magnitude, x-1, y-1) * weight < getColorAt(pixels_magnitude, x, y)
                   and getColorAt(pixels_magnitude, x+1, y+1) * weight < getColorAt(pixels_magnitude, x, y)):
                    pixels_borders[y][x] = 255
            elif(convertToRad(67.5) < pixels_direction[y][x]):
                if(getColorAt(pixels_magnitude, x, y-1) * weight < getColorAt(pixels_magnitude, x, y)
                   and getColorAt(pixels_magnitude, x, y+1) * weight < getColorAt(pixels_magnitude, x, y)):
                    pixels_borders[y][x] = 255
...
def HoG(pixels_magnitude : np.ndarray, pixels_direction : np.ndarray, interval_size : int) -> np.ndarray:
    histogram = np.zeros(floor(180/interval_size), dtype=np.float64)
    for y in range(0, len(pixels_magnitude)):
        for x in range(0, len(pixels_magnitude[0])):
            # Lembrar que o ângulos são apenas em valores de -pi/2 até pi/2
            angle = degrees(pixels_direction[y][x] + np.pi/2)
            index_aprox = int(floor(angle/interval_size))
            if(index_aprox < angle/interval_size and index_aprox != len(histogram)-1):
                histogram[index_aprox] += pixels_magnitude[y][x]/2
                histogram[index_aprox+1] += pixels_magnitude[y][x]/2
            else:
                histogram[index_aprox] += pixels_magnitude[y][x]
    return histogram

def showHoG(histogram):
    X = np.zeros(len(histogram))
    interval_size = 180.0 / float(len(histogram))
    for i in range(0, len(histogram)):
       X[i] = i * interval_size - 90
    plt.bar(X, histogram, color = 'maroon', width=1)
    plt.xlabel("Ângulo")
    plt.ylabel("Magnitude")
    # axis = plt.gca()
    # axis.set_ylim([max(f for f in histogram), max(f for f in histogram)])
...
if __name__ == "__main__":
    ...
    # gaussianBlur(image_path, image_ext, 7, 7, 3, True)
    ...
    pixels_border = edgeDectector(pixels_magnitude, pixels_direction, 1)

    histogram = HoG(pixels_magnitude, pixels_direction, 1)
    print(histogram)
