...
def colorizeImagePerLevel(pixels, colored_pixels, level, tolerance):
	level = (level/300) * 255
	for i in range(0, len(pixels)):
		for j in range(0, len(pixels[0])):
			if(abs(pixels[i][j][2] - level) <= tolerance):
				colored_pixels[i][j][0] = pixels[i][j][2]/4
				colored_pixels[i][j][1] = pixels[i][j][2]/4
				colored_pixels[i][j][2] = pixels[i][j][2]
			else:
				colored_pixels[i][j] = pixels[i][j]
...
if __name__ == "__main__":
	...
	colorizeImagePerLevel(pixels, colored_pixels, 100, 20)
